...
However, for setting up `gpio_server.py` and/or `key_monitor.py` as system-wide daemons, it is recommended to the package to Python's global paths. The following is the recommended approach with a `.pth` file.

```
echo GPIOSVR_PARENT_PATH >> "/usr/local/lib/python$( python3 --version | grep -oP '\d\.\d{1,2}' )/dist-packages/xoocoon.pth"
python -m site
```
...
The following is one possible way to do this.

```
perl -pe 's~^PATH=("?)(.+?)("?)$~PATH=\1\2:GPIOSVR_PATH\3~g' ~/.profile
```
